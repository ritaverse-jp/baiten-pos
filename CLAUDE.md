...
**商品写真（要件定義 v1.4 の 6.2・design 9章）はタスク21〜24をすべて実装済み**（21 GAS側・22 画像キャッシュ層・23 SC-03登録UI・24 会計画面への表示）。会計画面では**商品タイルの背景**と**伝票行のサムネイル（40px角）**の2箇所に出る。**商品タイルは写真の有無で寸法が変わらない**（写真は絶対配置で敷き、文字の下地は padding と同値の負 margin でレイアウトへの寄与を0にしている）。写真が無い商品は従来どおりの白カードで、混在しても一覧の行が揃う。 タスク21・22はユーザーが実機で動作確認済み（写真の差し替えでDrive上の旧ファイルがゴミ箱に移り、`productImages` の古いレコードが消えることまで確認）。
...
- `src/domain/categoryColor.ts` — SC-01 のカテゴリ別配色（要件定義7.3のコントラスト比4.5:1を機械的に保証。タスク20）。`resolveCategoryPalette(categories, categoryName)` がタブ・タイルの背景色/文字色を返す。管理者が `Category.color` を設定していればそこから導出（WCAG輝度計算で黒/白いずれか読みやすい方を自動選択。任意の色に対しても4.5:1以上になることを理論上・テストの両方で確認済み）、未設定なら `displayOrder` 順に固定パレットを割り当てる。**色に関わるUIを新しく追加するときは、素の色を直接使わず必ずこのモジュールを経由すること**。**商品写真の上に文字を載せる場合の下地（`photoScrim`／`PHOTO_SCRIM_ALPHA`）もここにある。** 写真は管理者が自由に登録するため色を事前に知れず、文字を直接重ねると4.5:1を保証できない。白の下地を不透明度 a で敷くと最悪ケース（写真が純黒）の背景色が `a×255` に定まるため、a を決めれば比が計算できる（`worstCaseScrimContrast`）。現在 0.88。**この値を下げるときは必ずテストで4.5:1を満たすことを確認すること**（0.5 まで下げると割ることをテストで明示してある）
...
- `src/state/productImageUrls.ts` — 商品写真の表示用 object URL をアプリ全体で1本ずつ共有する（`useProductImageUrl(imageId)`）。**写真を表示する画面は必ずこれを使い、`URL.createObjectURL` を各画面で直接呼ばないこと。** 同じ写真が商品タイルと伝票行に同時に出るうえ、伝票行は個数変更のたびに再描画されるため、都度作る作りだと object URL が際限なく増える。内部で `subscribeProductImages()` を購読しており、**バックグラウンドで届いた写真が自動で画面に反映される**。写真を差し替えると Drive のファイルIDが変わる設計なので「同じIDで中身だけ変わる」ことはなく、ID の増減だけ見ればよい。戻り値の `null` は異常ではない（未設定・未取得）ので、呼び出し側は写真なしの見た目にフォールバックし操作は制限しないこと（要件定義 9.1）。**`__resetProductImageUrlsForTests` は DB 側の購読解除も行う**——怠るとテストごとに購読が積み上がり、1回の更新で古い `rebuild` が何本も同時に走って同じ画像に複数の URL が作られる（実際にテストが落ちた）
- `src/data/image/resize.ts` — 商品写真の縮小（design 9.2・タスク23）。**`imageOrientation: 'from-image'` は未対応のブラウザだと不正な列挙値として `TypeError` になる**（iOS Safari では通るのに PC ブラウザで写真を追加できない不具合が実際に出た）。指定できなければオプションなしで一度だけ再試行する。**`domain/` ではなく `data/` に置く**（`canvas`・`createImageBitmap` というブラウザ API に依存するため。`domain/` は React にも通信にも DOM にも依存させない）。長辺320px・JPEG品質0.75。**アップロード前に必ずここを通すこと。** JPEG は透過を表現できないため、白で下地を敷いてから描画している（透過部分が黒く落ちるのを防ぐ）。**`createImageBitmap`・`canvas.toDataURL` は jsdom に無いため自動テストでは検証できない**（`ProductImage.test.tsx` はこのモジュールをモックしており、縮小そのものの正しさは実ブラウザでの確認が必要）
- `src/data/db/productImages.ts`・`src/data/sync/productImages.ts` — 商品写真のローカルキャッシュと取得制御（design 9.4・タスク22）。**写真の取得はバックグラウンドで GAS への往復（1枚あたり数秒）を挟むため、画面を描いた後に完了する。** これを知らせる手段が無いと画面は「開いた瞬間にキャッシュに有ったものだけ」を表示して固まる（実際にこれで「保存は成功しているのにプレビューに写真が出ない」不具合になった）。`subscribeProductImages()` でキャッシュの更新を購読し、届いたら読み直すこと。**写真を表示する画面を新しく作るときは必ずこれを購読する**（`putProductImage`・`deleteUnreferencedImages`・`deleteProductImageFromCache`・`clearProductImages` が通知する）。購読の仕組みを Zustand ではなく `data/db/` 内に置いているのは、`data/db/` を `state/` に依存させないため（design 3.2）。**写真の保存に成功したら、送ったバイト列をその場でキャッシュに書く**（`ProductsScreen.applyImageAction`）——手元にあるものを GAS から取り直す理由がない。**`syncProductImages` は fire-and-forget で呼ばれるため、絶対に例外を投げてはいけない。** 応答の `imageBase64` が壊れていると `atob` が投げ、未処理の Promise 拒否になるうえ残りの写真の取得まで巻き添えで止まる（実装当初これで実際に落ちていた）。1枚ごとに try/catch で囲み、失敗した1枚だけ諦めて次へ進む。**主キーは画像ID（Drive のファイルID）で、写真を差し替えると GAS 側が新しいファイルを作るためIDが変わる。したがって「IDが一致する＝中身も同じ」が常に成り立ち、キャッシュの鮮度判定を持たなくてよい**（端末は「マスタが参照しているIDのうち手元に無いものだけ」を取る）。**値は `Blob` ではなく `ArrayBuffer` ＋ `mimeType` で保存する**——IndexedDB は仕様上 Blob を格納できるが iOS Safari に既知の不具合があり、オフライン必須の本アプリでは採れない（`fake-indexeddb` でも Blob は構造化クローンを通らず `{}` になるため、テストでも検知できない形になっていた）。`syncProductImages` は `masterStore.refreshFromServer()` の成功後に **await せずに** 呼ばれる（写真は補助表示であり、取得を待って商品一覧を遅らせない。要件定義 9.1）。**通信失敗時は例外を投げずその場で打ち切り、残りは次回に持ち越す。** 1回の上限は20枚
