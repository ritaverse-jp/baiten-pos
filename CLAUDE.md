...
`docs/design.md` 7章の実装タスクのうち、**タスク20（PWA仕上げ）に着手中。7章のタスク一覧としては最後**。manifest・アイコン・precache・GitHub Pages配信・確認ダイアログ類の網羅・端末名変更機能・登録リセット機能までは実装・自動テスト・デプロイ済み。**「圏外でホーム画面から起動し会計を完了できる」の実機での最終確認はまだ**。

**未解明の問題：`端末` タブの行が理由不明で失われた事象が実機検証中に発生した。** トークンのハッシュ照合は通る（＝Script Propertiesのトークン実体は存在する）のに、対応する行が `端末` タブに1件も無く `TERMINAL_DISABLED` になる状態を確認した。GAS 側のコードには `端末` タブの行を削除する処理が無く（`renameTerminal` もB列のみ書き換え）、原因は特定できていない。復旧手段として設定画面に「登録をやり直す（リセット）」ボタンを追加した（下記 `screens/settings/` の項参照）が、**根本原因の調査は持ち越し**。今後 `端末` タブのデータが消える事象が再発したら、このメモを参照し、GAS 側のログ（実行ログ・`操作ログ`タブ）を先に確認すること。
