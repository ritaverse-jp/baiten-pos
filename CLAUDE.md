...
**ユーザー承認済み・タスク20完了後に着手する対応（未実装）：**
1. `requireAuth_` で「ハッシュは有効だが `端末` タブに行が無い」場合に、`TERMINAL_DISABLED` ではなく専用エラー `TERMINAL_NOT_REGISTERED` を返す
2. アプリ側でそのエラーを受けたとき「登録をやり直してください」と案内し、設定画面のリセットボタンへ誘導する

（「シートに注意書きを置く」案も出したが、ユーザー判断で今回は見送り。）復旧手段として設定画面に追加済みの「登録をやり直す（リセット）」ボタンは下記 `screens/settings/` の項を参照。
