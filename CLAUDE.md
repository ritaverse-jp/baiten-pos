...
`docs/design.md` 7章の実装タスクのうち、**タスク19（SC-06 設定・初回セットアップ）まで完了**。7章のタスク一覧としては最後（残るはタスク20：PWA仕上げ）。
