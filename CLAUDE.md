...
**商品写真（要件定義 v1.4 の 6.2・design 9章）は タスク21（GAS側）・22（端末側の画像キャッシュ層）まで実装済み。タスク23（登録UI）・24（会計画面への表示）は未着手**（ユーザーが21・22の動作確認をしてから着手を判断する方針）。**この時点では写真を登録する画面がまだ無いため、動作確認には `saveProductImage` を curl か コンソールから直接呼ぶ必要がある。**
