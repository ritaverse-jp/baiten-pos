...
`docs/design.md` 7章の実装タスクのうち、**タスク18（SC-05 履歴＋取消）まで完了**。次はタスク19（SC-06 設定・初回セットアップ）。
...
**GAS はデプロイ更新済み（バージョン11）。** `getSalesHistory`・`cancelSale` を curl で疎通確認済み（無認証・不正トークンの両方で正しく `UNAUTHORIZED` が返り、action 自体は正しくルーティングされていることを確認）。**実トークンでの一気通貫確認（実際に会計を取消してシートに負の行が追記されることの確認）はユーザー側の実ブラウザ操作で行う予定**（タスク17と同じ運用）。

**A列（日付）・B列（時刻）をコードから読み返す実装は今回が初めて。** これまでの GAS 実装（`appendSales`・`getTodayMaxSeq`）は売上ログに書き込むだけ、または C列（会計番号・文字列)を読むだけで、日付・時刻セルを読み返す処理が無かった。日付・時刻の文字列（`Utilities.formatDate` で書き込んだもの）はスプレッドシート側の自動書式認識により Date 型セルとして保存され得るため、`getSalesHistory` の実装では `cellDateString_`/`cellTimeString_`（`gas/Sales.js`）で Date 型・文字列型のどちらでも正しく扱えるようにしている。**これは静的な推測に基づく防御的実装であり、実際にどちらの型で保存されるかは実データでまだ確認していない。** 日付が正しく絞り込めているか（＝別日の会計が混ざらないか）は実ブラウザでの確認時に注意して見ること。
...
**まだ対応できていないこと：** `data/sync/counter.ts` の `reconcileCounterOnStartup` は実装済みだが、呼び出し元が無い。design 5.3 の文言（「起動時に...検知したら」）どおり、本来は `app/App.tsx` の起動時 `hydrate()` 群と同じタイミングで呼ぶのが自然（タスク15の会計確定のたびに呼ぶものではない）。`'blocked'` が返ったら会計開始をブロックしてその旨を表示する必要がある。着手時にこの CLAUDE.md を検索して見つけること。
