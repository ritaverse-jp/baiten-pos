...
**商品写真（要件定義 v1.4 の 6.2・design 9章・タスク21〜24）は設計だけ済んでおり、実装は未着手。** ユーザーからの「商品設定の際に写真も入れられるようにできるか」という質問を受けて要件定義・設計書に落とした段階。**保存先は Google Drive（シートには画像IDのみ）**で、画像の実体をスプレッドシートに置かない理由・端末側で320pxに縮小してから送る理由・`getMasters` に画像を混ぜてはいけない理由は design 9.1〜9.3 に書いてある。着手前に必ず design 9章を読むこと。**タスク24（SC-01への写真表示）は、タスク20で調整したレイアウト寸法（`--tile-min` 等）の再調整を伴う。**

`docs/design.md` 7章の実装タスクのうち、**タスク20（PWA仕上げ）に着手中。タスク1〜20としては最後**（21〜24は商品写真として後から追加したもので、20とは独立）。manifest・アイコン・precache・GitHub Pages配信・確認ダイアログ類の網羅・端末名変更機能・登録リセット機能までは実装・自動テスト・デプロイ済み。**「圏外でホーム画面から起動し会計を完了できる」の実機での最終確認はまだ**。
